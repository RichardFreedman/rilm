...
#         df['year'] = df['year'].fillna("1960")

#         # year_min, year_max = st.slider('Year Range', min_value=df['year'].min(), max_value=df['year'].max())
#         # df = df[(df['year'] >= year_min) & (df['year'] <= year_max)]

#         category = st.multiselect('Category', df['category'].unique())
#         author = st.multiselect('Author', df['author'].unique())
#         df = df[(df['category'] == category) & (df['author'] == author)]
...
#     modify = st.sidebar.checkbox("Filter the Results")
...
#         to_filter_columns = st.multiselect("Filter results according to values in each column.", df.columns)
...
#                 increment = 1 # Set the increment here
#                 user_num_input = right.slider(
#                     f"Years for {column}",
#                     _min,
#                     _max,
#                     (_min, _max),
#                     step=increment, # Set the increment here
#                 )
#                 df = df[df[column].between(*user_num_input)]

            # if is_categorical_dtype(df[column]) or df[column].nunique() < 400:
            ...
            # elif is_numeric_dtype(df[column]) and column != 'year':
            #     _min = int(df[column].min())
            #     _max = int(df[column].max())
            #     user_num_input = right.slider(
            #         f"Values for {column}",
